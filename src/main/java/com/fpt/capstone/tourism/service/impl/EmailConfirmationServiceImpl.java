...
public class EmailConfirmationServiceImpl implements EmailConfirmationService {
    ...
    public void sendConfirmationEmail(User user, Token token) {
        try {
            ...
            String link = frontendBaseUrl + "/confirm-email?token=" + token.getToken();
            String subject = "Xác Nhận Email Viet Travel";
            String content = "Kính gửi " + user.getFullName() + ",\n\n"
                    + "Chào mừng bạn đến với Đi Đâu! Chúng tôi rất vui mừng khi bạn tham gia cộng đồng của chúng tôi."
    ...
    public void sendForgotPasswordEmail(User user, Token token) {
        try {
            String link = backendBaseUrl + "/reset-password?token=" + token.getToken();
